...
fn process_decoded_string(src:String, operation:i32) -> String { // operation = 0 -> decode, operation = 1 -> remove
    let mut ret:String = "".to_string();
    let mut i = 0;
    while i < src.chars().count() {
        if get_char(src.to_string(), i) == '%' {
            if operation == 0 { // ako decodujemo, onda lepimo dekodovan znak na rezultujuci strings. U suportnom samo preskocimo
...
fn URLdecode(src:String) -> String { // decodes URL encoded parts of the string
   return process_decoded_string(src, 0);
}

fn URLremove(src:String) -> String { // remove URL encoded parts of the string
...
fn main() {
    ...
    let url_decoded_request:String = URLremove(request);
